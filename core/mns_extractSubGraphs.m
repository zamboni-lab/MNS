...
%
function mnsSubGraphResults = mns_extractSubGraphs(model,dataStruct, nameTag, nlRange, initMNS, plotResults)
t1 = tic;
...
if max(dataStruct.data) <= 1 && min(dataStruct.data) >= 0
    if ~initMNS.subGraphExtraction.noLogTransform
...
%         copyfile([mnsFolder 'nCliqueNoPerm.txt'], [mnsDataFolder '\coarse_nl_' num2str(nlTemp) '\nCliqueNoPerm.txt']);
%         disp(clFolder);
%         disp([clFolder 'nCliqueNoPerm.txt'])
%         disp([mnsDataFolder '\coarse_nl_' num2str(nlTemp) '\nCliqueNoPerm.txt']);
...
%         disp([mnsDataFolder 'coarse_nl_' num2str(nlTemp) '\'])
        [mns] = mns_run2state(model, dataStruct, initMNStemp, mnsSubDataFolder, mnsExecFolder);
        if isempty(mns.results)
            mnsSubGraphResults = [];
            return;
        end
%         mnsSubGraphResults.mns(i) = {mns};
%         mnsSubGraphResult.breakingReaction(:,i) = mns.results.breakingReactions;
%         mnsSubGraphResult.clustLabel(:,i) = mns.results.inferedLabels;
        if i == 1
...
for s = 1:length(initMNS.subGraphExtraction.std0Steps) % scan through std values
    ...
    for i = 1:length(nlRange)
        ...
        [mns] = mns_run2state(model, dataStruct, initMNS, mnsSubDataFolder, mnsExecFolder);
        if isempty(mns.results)
            mnsSubGraphResults = [];
            return;
        end
        ...
        [subGraphs, listTemp] = getSubgraphs(mns.results.breakingReactions, mns.results.reactionPairs, model.iaMat, mns.observations.iaId2dataId, mns.results.inferedLabels);
        if ~isempty(listTemp)
...
mnsSubGraphResults.subGraphListHeader = {'SubGraphId','std0', 'std1', 'nL1' 'settingsSubGraphId', '#Metabolites', 'Model MetId', '#NoEdges', 'Edges', 'EdgeIdx', '#Detected Metabolites','#unique Ions', 'Unique Ions Idx'};
...
function [subGraphs, subGraphList] = getSubgraphs(breakingReaction, ...
    reactionPairs, iaMat, iaId2DataId, inferedLabels)
...
subGraphs.metSubGraphLabels = ones(size(iaMat,1),1)*-1;
cMetLabel = 0;
subGraphList = [];
while(min(subGraphs.metSubGraphLabels) == -1)
    idx = find(subGraphs.metSubGraphLabels == -1);
    idx = idx(1);
    if inferedLabels(idx) == 0 
        subGraphs.metSubGraphLabels(isinf(sPaths(idx,:)) == 0) = 0;
    else
        tempMetIdx = find(isinf(sPaths(idx,:)) == 0);
        temp = iaId2DataId(tempMetIdx);
        temp(temp == 0) = [];
        if ~isempty(temp) && length(temp) > 1
            cMetLabel = cMetLabel+1;
            subGraphs.subGraphDetails(cMetLabel).metIdx = find(isinf(sPaths(idx,:)) == 0);
            % get edges
            edges = [];
            edgeIdx = [];
            for i = 1:length(subGraphs.subGraphDetails(cMetLabel).metIdx)
                idxTemp = find(iaMat(subGraphs.subGraphDetails(cMetLabel).metIdx(i),:) ~= 0);
                idxTemp(idxTemp < subGraphs.subGraphDetails(cMetLabel).metIdx(i)) = [];
                edges = [edges; ones(length(idxTemp),1)*subGraphs.subGraphDetails(cMetLabel).metIdx(i) idxTemp'];
            end
            edgeIdx = [(edges(:,1)-1).*size(iaMat,1)+edges(:,2)];
            
            subGraphs.subGraphSize(cMetLabel) = length(subGraphs.subGraphDetails(cMetLabel).metIdx);
            subGraphs.metSubGraphLabels(subGraphs.subGraphDetails(cMetLabel).metIdx) = cMetLabel;
            temp = iaId2DataId(subGraphs.subGraphDetails(cMetLabel).metIdx);
            subGraphs.subGraphDetails(cMetLabel).dataMetIdx = unique(temp(temp ~= 0));
            subGraphs.subGraphUniqueIons(cMetLabel) = length(subGraphs.subGraphDetails(cMetLabel).dataMetIdx);
            subGraphs.subGraphDetectedMetabolites(cMetLabel) = length(temp(temp ~= 0));
            subGraphs.subGraphDetails(cMetLabel).edges = edges;
            subGraphs.subGraphDetails(cMetLabel).edgeIdx = edgeIdx;
            subGraphs.subGraphNoEdges(cMetLabel) = length(edgeIdx);
            subGraphList = [subGraphList; cMetLabel subGraphs.subGraphSize(cMetLabel) ...
                {subGraphs.subGraphDetails(cMetLabel).metIdx} subGraphs.subGraphNoEdges(cMetLabel)...
                {subGraphs.subGraphDetails(cMetLabel).edges} {subGraphs.subGraphDetails(cMetLabel).edgeIdx}...
                subGraphs.subGraphDetectedMetabolites(cMetLabel)...
                subGraphs.subGraphUniqueIons(cMetLabel) {subGraphs.subGraphDetails(cMetLabel).dataMetIdx}];
        else
            subGraphs.metSubGraphLabels(isinf(sPaths(idx,:)) == 0) = 0;
        end
    end
end
% subGraphs.metSubGraphLabels((subGraphs.metSubGraphLabels) == -1) = 0;
...
mergedSubGraphStruct.metSubGraphLabels = zeros(length(mns.observations.metId),1);
noEdges = cell2mat(subGraphList(:,8));
edgeIdx = subGraphList(:,10);
nMet = size(iaMat,1);
...
%             disp(i)
    ...
    for i = 1:length(tempStruct)
        ...
        mergedSubGraphStruct.subGraphEdges(c)= length(edgeIdxFrequency);
        ...
        % get metabolite Ids from edges
        mergedSubGraphStruct.subGraphDetails(c).edges = [(edgeIdxUnion-mod(edgeIdxUnion,nMet))/nMet+1, mod(edgeIdxUnion,nMet)];
        mergedSubGraphStruct.subGraphDetails(c).metIdx = ...
            unique(reshape(mergedSubGraphStruct.subGraphDetails(c).edges,length(edgeIdxUnion)*2,1));
        mergedSubGraphStruct.subGraphSize(c) = length(mergedSubGraphStruct.subGraphDetails(c).metIdx);
        ionIdxUnion = mns.observations.iaId2dataId(mergedSubGraphStruct.subGraphDetails(c).metIdx);
        ...
        mergedSubGraphList = [mergedSubGraphList; ...
            ...
            mean(edgeIdxFrequency) {edgeIdxUnion} {mergedSubGraphStruct.subGraphDetails(c).edges}...
            {edgeIdxFrequency} ...
            mergedSubGraphStruct.subGraphSize(c) {mergedSubGraphStruct.subGraphDetails(c).metIdx}...
            mergedSubGraphStruct.subGraphDetectedMetabolites(c)...
            mergedSubGraphStruct.subGraphUniqueIons(c)...
            {mergedSubGraphStruct.subGraphDetails(c).ionIdxUnion}];
    end
end
mergedSubGraphHeader = {'SubGraphIdx' '#Edges' '#Core Edges' 'AvgFreq' 'EdgeIdx' 'Edges' 'EdgeFreq' '#Metabolites' 'MetIdx'...
     '#Detected Metabolites', '#unique Ions' 'ionIdx'};
end
...
while(min(subGraphLabels) == 0)
    ...
    mergedSubGraphStruct.metSubGraphLabels(metIdxUnion) = c;
